...
function App() {
  ...
  const [filteredItems,setfilterdItems] = useState([item])
  const [selectItem, setselectItem] = useState("");
  const [filter, setFilter] = useState("");
  let handleClick = () => {
    if (selectItem.trim().length === 0) {
      alert(`Opps!...  Add the item please`);
      return;
    }
    let newobj = {};
    newobj.id = ++count;
    newobj.taskname = selectItem;
    newobj.taskdone = false;
    setItems([...item, newobj]);
    setselectItem("");
    setfilterdItems([...item])
  };
  let handleDelete = (id) => {
    const arr = item.filter((ele) => {
      return ele.id !== id;
    });
    setItems(arr);
  };
  let handleCheck = (ele) => {
    let index = item.indexOf(ele);
    let obj = [...item];
    obj[index] = { ...ele, taskdone: true };
    setItems(obj);
  };
  useEffect(() => {
  //  setfilterdItems([...item])
     let updateditems = [ ];
     if(filter==="Completed"){
       updateditems = item.filter((ele)=>{
            return ele.taskdone===true;
       })
       console.log("com");
   ...
   let renderItems = filteredItems.length > 0 ? filteredItems : item;
  return (
    <>
            ...
            renderItems.map((item, index) => {
              return (
                <div key={index} className="lists">
                  <h6 className={item.taskdone ? "stike" : ""}>
                    {item.taskname}
                  </h6>
